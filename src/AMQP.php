...
class AMQP
{
    ...
    {
        $tmpFlags = 0;
        if (array_get($this->config, 'queue.passive')) {
            $tmpFlags |= AMQP_PASSIVE;
        }
        if ($this->queueName === '' ? false : array_get($this->config, 'queue.durable')) {
            $tmpFlags |= AMQP_DURABLE;
        }
        if ($this->queueName === '' ? true : array_get($this->config, 'queue.exclusive')) {
            $tmpFlags |= AMQP_EXCLUSIVE;
        }
        if (array_get($this->config, 'queue.auto_delete')) {
    ...
    public function exchange($exchangeName = '', $flags = AMQP_NOPARAM)
    {
        ...
        $tmpFlags = 0;
        if (array_get($this->config, 'exchange.passive')) {
            $tmpFlags |= AMQP_EXCLUSIVE;
        }
        if (array_get($this->config, 'exchange.durable')) {
            $tmpFlags |= AMQP_DURABLE;
        }
        if (array_get($this->config, 'exchange.auto_delete')) {
            $tmpFlags |= AMQP_AUTODELETE;
        }
        if (array_get($this->config, 'exchange.internal')) {
    ...
    public function initConnection()
    {
        ...
        $connection = new AMQPConnection(array_get($this->config, 'connection'));
        //$connection->setHost(array_get($this->config, 'connection.host'));
        //$connection->setPort(array_get($this->config, 'connection.port'));
        $connection->setLogin(array_get($this->config, 'connection.user'));
        //$connection->setPassword(array_get($this->config, 'connection.password'));

        if (empty($this->vhost)) {
            $connection->setVhost(array_get($this->config, 'connection.vhost', '/'));
        } else {
    ...
    protected function clearVhost()
    {
        if (!is_null($this->connection) && !empty($this->vhost)) {
            $this->vhost(array_get($this->config, 'connection.vhost', '/'));
        }
    }
    ...
    public function initQueue()
    {
        ...
        $queue->setArguments(array_get($this->config, 'queue.arguments'));
    ...
    protected function log($message, $attributes, $context)
    {
        ...
        $this->log->log(
            array_get($attributes, 'log_level', $this->config['default_log_level']),
            $message,
            $context
        );
    }
    ...
    protected function createOrGetExchange()
    {
        ...
        $exchange->setType(array_get($this->config, 'exchange.type'));
        $exchange->setName($this->exchangeName);
        $exchange->setFlags($this->exchangeFlags);
        $exchange->setArguments(array_get($this->config, 'exchange.arguments', []));
        return $exchange;
    }
    ...
    {
        if (!$this->rebuild && ($heartbeat = array_get($this->config, 'connection.heartbeat', 0)) > 0) {
            if ($heartbeat > 60) {
                $heartbeat = 60;
            }
